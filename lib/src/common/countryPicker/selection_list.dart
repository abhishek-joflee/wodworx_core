...
class _SelectionListState extends State<SelectionList> {
      ...
      body: Container(
        ...
        child: LayoutBuilder(
          builder: (context, constraints) {
            ...
            _heightScroll = (constraints.biggest.height) / _alphabet.length;
                  ...
                  slivers: [
                    ...
                    SliverList(
                      delegate: SliverChildBuilderDelegate(
                        (context, index) => widget.countryBuilder != null
  ...
  _getAlphabetItem(int index) => Expanded(
        child: InkWell(
          onTap: () {
            setState(() {
              posSelected = index;
    ...
    setState(() {
      if ((_offsetContainer + details.delta.dy) >= 0 &&
          (_offsetContainer + details.delta.dy) <=
              (_sizeHeightContainer - _heightScroll)) {
        _offsetContainer += details.delta.dy;
        posSelected =
            ((_offsetContainer / _heightScroll) % _alphabet.length).round();
        _text = _alphabet[posSelected];
        if (_text != _oldText) {
          for (var i = 0; i < countries.length; i++) {
            if (_text
